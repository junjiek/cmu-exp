#!/bin/bash
if [[ $# != 3 ]]; then
    echo "Usage: ./svmlight.sh <trainDataFile> <testDataFile> <id>"
    exit 0
fi
...
OUTDIR=$ROOTDIR/output/svmlight
BINARY=$ROOTDIR/tool/svmlight/binary.py

res=`(time -p python $BINARY -c 10 -v 0 $1 $2 $3) 2> "$OUTDIR/$3.time"`
