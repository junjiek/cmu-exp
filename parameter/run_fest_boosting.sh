...
round=16
for (( i = 4; i <= 10; i++ )); do
    echo 'featureSize,sampleSize,ExactAcc,microF,macroF,time' > fest_bt_$2_$round.out
    ...
    for trainfile in $1_*; do
        ...
        res=`(time -p python $BINARY -c 2 -d 5 -t $round -e $trainfile $testfile $2_$round) 2> "$OUTDIR/$2_$round.time"`
        ...
        echo $featureNum,$sampleNum,$outinfo,$realTime >> fest_bt_$2_$round.out
        num=`echo $num + 1 | bc`
    done;
