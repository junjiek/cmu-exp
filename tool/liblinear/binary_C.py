...
svmtrain_exe = '../tool/liblinear_C.sh'
svmpredict_exe = '../liblinear/build/predict'
outDir = '../output/liblinear/'
...
def train_problem(lab):
	...
	cmd = "%s %s %s" % (svmtrain_exe, outDir + sys.argv[-1] + "_tmp_binary", outDir + sys.argv[-1] + "_tmp_model")
	os.system(cmd)

def test_problem(lab):
	# print "Testing problem for label %s..." % lab

	cmd = "%s %s %s %s" % (svmpredict_exe, outDir + sys.argv[-1] + "_tmp_test", outDir + sys.argv[-1] + "_tmp_model", outDir + sys.argv[-1] + "_tmp_output")
	os.system(cmd)
