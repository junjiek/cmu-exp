...
festlearn_exe = '../fest/build/festlearn'
festSCut_exe = '../fest/build/festSCut'
festclassify_exe = '../fest/build/festclassify'
outDir = "../output/fest/b/"
...
def build_problem(lab):
    ...
    validation = open(outDir + sys.argv[-1] + "_tmp_validation", "w")
    tot = len(labels)
    rint = randint(0, tot)
    for t in range(tot):
        if lab in labels[t]:
            if (t - rint) % 5 == 0:
                validation.write("+1 %s\n" % features[t])
            else:
                problem.write("+1 %s\n" % features[t])
        else:
            if (t - rint) % 5 == 0:
...
def train_problem(lab):
    ...
    cmd = "%s %s %s %s" % (festlearn_exe, pass_through_options,
                            outDir + sys.argv[-1] + "_tmp_binary", outDir + sys.argv[-1] + "_tmp_model")
    os.system(cmd)

def find_threshold(lab):
    print "Validation problem for threhold %s..." % lab
    cmd = "%s %s %s %s" % (festSCut_exe, outDir + sys.argv[-1] + "_tmp_validation",
                            outDir + sys.argv[-1] + "_tmp_model", outDir + sys.argv[-1] + "_tmp_threshold")
    os.system(cmd)

def test_problem(lab):
    print "Testing problem for label %s..." % lab
    cmd = "%s %s %s %s %s" % (festclassify_exe, outDir + sys.argv[-1] + "_tmp_test",
                                  outDir + sys.argv[-1] + "_tmp_model", outDir + sys.argv[-1] + "_tmp_threshold",
                                  outDir + sys.argv[-1] + "_tmp_output")
    os.system(cmd)
