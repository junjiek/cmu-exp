...
sklearn_exe = 'python ../sklearn/randomforest.py'
outDir = '../output/sklearn/'
...
def process_options(argv = sys.argv):
    ...
    if len(argv) < 3:
        print "Usage: %s training_file testing_file predictionOutput" % argv[0]
        sys.exit(1)
...
def train_test_problem(lab):
    ...
    cmd = "%s %s %s %s %s 0" % (sklearn_exe, pass_through_options,
                              outDir + "tmp_binary", outDir + "tmp_test", outDir + "tmp_output")
    os.system(cmd)
