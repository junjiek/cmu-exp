...
svmtrain_exe = '../svm_light/svm_learn'
svmpredict_exe = '../svm_light/svm_classify'
outDir = '../output/svmlight/'
...
def test_problem(lab):
    ...
    cmd = "%s %s %s %s" % (svmpredict_exe, "-v 0 " + outDir + sys.argv[-1] + "_tmp_test", outDir + sys.argv[-1] + "_tmp_model", outDir + sys.argv[-1] + "_tmp_output2")
    os.system(cmd)

    cmd = "awk -F: '{if ($1 > 0) print 1; else print -1}' %s > %s" % (outDir + sys.argv[-1] + "_tmp_output2", outDir + sys.argv[-1] + "_tmp_output")
    os.system(cmd)
