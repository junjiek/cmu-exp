...
def process_options(argv = sys.argv):
    ...
    if len(argv) < 3:
        print "Usage: %s [parameters for fest-train] training_file testing_file" % argv[0]
        sys.exit(1)
...
def train_test_problem(lab):
    ...
    cmd = "%s %s %s %s %s %s" % (festmulticlass_exe, pass_through_options,
                                 outDir + "tmp_binary", outDir + "tmp_binary_header", outDir + "tmp_test", outDir + "tmp_output")
    os.system(cmd)
...
def main():
    ...
    bh = open(outDir + 'tmp_binary_header', 'w');
    bh.write('+1 -1\n');
    bh.close();
