...
void growForest(forest_t* f, dataset_t* d){
    ...
    f->nfeat = d->nfeat;
    f->tree = (node_t**)malloc(f->ntrees*sizeof(node_t*));
    tree.valid = (int*)malloc(d->nex*sizeof(int));
    tree.used = (int*)calloc(d->nfeat,sizeof(int));
    tree.feats = (int*)malloc(d->nfeat*sizeof(int));
    for(i=0; i<d->nfeat; i++)
        tree.feats[i]=i;
    tree.maxdepth = f->maxdepth;
    tree.committee = f->committee;
    tree.pred = (float*)malloc(d->nex*sizeof(float));
